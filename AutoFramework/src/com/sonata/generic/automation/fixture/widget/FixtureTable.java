...
import com.sonata.generic.automation.widgets.Table;
import com.sonata.generic.automation.widgets.Widget;

/**
 * A fixture-friendly representation of a table.
 */
public final class FixtureTable extends AbstractFixtureWidget
{
   private static final FixtureWidgetCreator CREATOR = new FixtureTableCreator();
    ...
    */
   private FixtureTable(final String widgetName, final String widgetID, final String idBase, final Browser browser)
   {
      super(widgetName, widgetID, idBase, browser);
      this.table = new Table(getLocator(), browser);
   }
    ...
    */
   private static class FixtureTableCreator implements FixtureWidgetCreator
   {
      /**
       * {@inheritDoc}
       */
      ...
      {
         return new FixtureTable(widgetName, widgetID, idBase, browser);
      }
   }
   ...
   private boolean clickCellByJavaScript(final int rowIndex, final int columnIdx)
   {
      ...
      // Just use click() of javascript, it will clear the previous cell's content if which is not tab out
    ...
    */
   @Override
   public boolean selectFromCellWithoutValidation(final int rowIndex, final int columnIdx, final String value)
   {
      // click the cell to activate the inner element (a textbox, or whatever it is)
      if (this.clickCellByJavaScript(rowIndex, columnIdx))
      {   
         // Can not do the text validation since App may change it back if validation fails
         // Suggest do the validation with getCellText in another step.
         return table.getComboBoxField(rowIndex, columnIdx).selectComboBox(value, false);
      }
      else
         return false;
   }
